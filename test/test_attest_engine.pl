...
engine_or_skip(B) :-
    (   engine_available(B) -> true
    ;   format(user_error, "~n  ~w not on PATH; engine attestation skipped~n", [B]),
        fail
    ).
...
test(pure_goals_are_pure, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)),
...
test(output_is_a_wire, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)), true(fires(V, output)) ]) :-
    verdict_of(B, write(hello), V).

test(a_new_operator_is_a_wire, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)), true(fires(V, ops)) ]) :-
    verdict_of(B, op(700, xfx, hg_probe_op), V).

test(a_changed_flag_is_a_wire, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)), true(fires(V, flags)) ]) :-
    verdict_of(B, set_prolog_flag(double_quotes, atom), V).

test(an_asserted_clause_is_a_wire, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)),
                                     true(fires_any(V, [dynclauses, preds])) ]) :-
    verdict_of(B, assertz(hg_probe_fact(1)), V).

test(an_opened_stream_is_a_wire, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)),
                                   true(fires_any(V, [streams, files])) ]) :-
    verdict_of(B, open('hg_probe_left_open.txt', write, _), V).

test(a_written_file_is_a_wire, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)),
                                 true(fires(V, files)) ]) :-
    verdict_of(B, ( open('hg_probe_written.txt', write, S), close(S) ), V).

test(an_undefined_predicate_is_reported_not_passed, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)),
                                                       true(V == not_defined) ]) :-
...
test(a_runaway_goal_is_bounded_not_a_hang, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)),
...
test(the_full_profile_is_clean, [ forall(member(B, [scryer, trealla])), condition(engine_or_skip(B)), true(Bad == []) ]) :-
