...
MUTATIONS = [
    ...
    ("head_may_shadow_trusted", "a clause head may redefine a host-trusted predicate",
     "    ;   hg_trusted(Ind, Ctx, _)\n    ->  throw(hg_refused(permission_error(modify, static_procedure, Ind), escape_attempt, head(trusted)))",
     "    ;   fail, hg_trusted(Ind, Ctx, _)\n    ->  throw(hg_refused(permission_error(modify, static_procedure, Ind), escape_attempt, head(trusted)))"),
    ("evaluables_not_checked", "arithmetic expressions are not checked for pinned evaluables",
     "    hg_indicator(G, Name/Arity, D, Ctx, N0, N),\n    hg_check_evaluables(G, Name/Arity, D).",
     "    hg_indicator(G, Name/Arity, D, Ctx, N0, N),\n    true."),
    ("runtime_call_not_judged", "hornguard_call/1 calls its goal without judging it",
     "hornguard_call(Goal) :-\n    hg_strip_module(Goal, M, G),\n    hg_runtime_judge(G, Verdict),\n    hg_runtime_proceed(Verdict, M, G).",
     "hornguard_call(Goal) :-\n    hg_strip_module(Goal, M, G),\n    call(M:G)."),
    ("guard_leaves_unbound_goal", "the judged-mode rewrite leaves an unbound goal in place",
